...
def solverSW(startRes, approach, getData, getError, useAdjoint, aposteriori, mode='tohoku', op=opt.Options()):
    ...
    if getData:
        ...
        if approach in ('implicit', 'DWE'):
            ...
            B_, L = form.formsSW(q_oi, q_oi_, b, Dt, coriolisFreq=f, neumann=True)
            B = form.formsSW(e, e_, b, Dt, neumann=True)[0]
        ...
        while t <= T:
            if cnt % op.rm == 0:
                ...
                adaptProblem = NonlinearVariationalProblem(
                    form.weakResidualSW(q, q_, b, Dt, coriolisFreq=f, neumann=True, op=op), q)
